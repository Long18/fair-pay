...
COMMENT ON FUNCTION "public"."bulk_delete_expenses"("p_expense_ids" "uuid"[]) IS 'Deletes multiple expenses atomically. Max 50 expenses at a time.
...
CREATE OR REPLACE FUNCTION "public"."get_balance_history"("p_user_id" "uuid" DEFAULT NULL::"uuid", "p_start_date" "date" DEFAULT (CURRENT_DATE - '30 days'::interval), "p_end_date" "date" DEFAULT CURRENT_DATE, "p_currency" "text" DEFAULT 'USD'::"text") RETURNS TABLE("snapshot_date" "date", "total_owed" numeric, "total_lent" numeric, "net_balance" numeric, "currency" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_user_id UUID;
...
COMMENT ON FUNCTION "public"."get_balance_history"("p_user_id" "uuid", "p_start_date" "date", "p_end_date" "date", "p_currency" "text") IS 'Retrieves historical balance data for trend charts. Auto-calculates missing snapshots on-demand.';
...
CREATE OR REPLACE FUNCTION "public"."get_public_demo_debts"() RETURNS TABLE("counterparty_id" "uuid", "counterparty_name" "text", "amount" numeric, "i_owe_them" boolean, "owed_to_name" "text", "owed_to_id" "uuid")
    LANGUAGE "plpgsql" SECURITY DEFINER
...
COMMENT ON FUNCTION "public"."get_public_demo_debts"() IS 'Returns REAL production debt data from user with most debts. Automatically updates when data changes. No hardcoded user ID required.';



CREATE OR REPLACE FUNCTION "public"."get_public_recent_activities"("p_limit" integer DEFAULT 10, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" "uuid", "type" "text", "description" "text", "amount" numeric, "currency" "text", "date" timestamp with time zone, "group_id" "uuid", "group_name" "text", "created_by_id" "uuid", "created_by_name" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
COMMENT ON FUNCTION "public"."get_public_recent_activities"("p_limit" integer, "p_offset" integer) IS 'Returns recent expense activities from public groups. Accessible to everyone including unauthenticated users. Does not include payments for privacy.';
...
COMMENT ON FUNCTION "public"."get_top_categories"("p_start_date" "date", "p_end_date" "date", "p_group_id" "uuid", "p_limit" integer) IS 'Returns top spending categories with amounts and percentages for analytics dashboard.';
...
CREATE OR REPLACE FUNCTION "public"."get_user_activities"("p_user_id" "uuid", "p_limit" integer DEFAULT 20) RETURNS TABLE("id" "uuid", "type" "text", "description" "text", "total_amount" numeric, "currency" "text", "date" timestamp with time zone, "group_id" "uuid", "group_name" "text", "created_by_id" "uuid", "created_by_name" "text", "paid_by_id" "uuid", "paid_by_name" "text", "user_share" numeric, "is_lender" boolean, "is_borrower" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."get_user_activities"("p_user_id" "uuid", "p_limit" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_user_activities"("p_user_id" "uuid", "p_limit" integer) IS 'Returns expenses and payments for a specific user with detailed split information.
...
CREATE OR REPLACE FUNCTION "public"."get_user_debts_aggregated"("p_user_id" "uuid" DEFAULT "auth"."uid"()) RETURNS TABLE("counterparty_id" "uuid", "counterparty_name" "text", "amount" numeric, "i_owe_them" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION "public"."get_user_debts_history"("p_user_id" "uuid" DEFAULT "auth"."uid"()) RETURNS TABLE("counterparty_id" "uuid", "counterparty_name" "text", "total_amount" numeric, "settled_amount" numeric, "remaining_amount" numeric, "i_owe_them" boolean, "transaction_count" bigint, "last_transaction_date" "date")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
BEGIN
...
COMMENT ON FUNCTION "public"."get_user_debts_history"("p_user_id" "uuid") IS 'Returns all debt relationships for a user including settled debts, ordered by outstanding balance then recency. Recreated after migration 033 dropped it via CASCADE.';
...
COMMENT ON FUNCTION "public"."settle_all_group_debts"("p_group_id" "uuid") IS 'Settles all outstanding debts in a group. Only group admins can perform this operation.
...
COMMENT ON FUNCTION "public"."settle_split"("p_split_id" "uuid", "p_amount" numeric) IS 'Settle an individual split with optional custom amount';
...
COMMENT ON FUNCTION "public"."soft_delete_expense"("p_expense_id" "uuid") IS 'Soft delete an expense (creator only)';
...
COMMENT ON FUNCTION "public"."unsettle_split"("p_split_id" "uuid") IS 'Unsettle a split (for corrections)';
...
GRANT ALL ON FUNCTION "public"."get_user_activities"("p_user_id" "uuid", "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_activities"("p_user_id" "uuid", "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_activities"("p_user_id" "uuid", "p_limit" integer) TO "service_role";
