...
import {
  ...
  getSampleDestinationForTemplate,
  getRefRows,
  extractRefCode,
  eventBadgeVariant,
} from "./helpers";

export function MetricTable({ title, description, rows }: { title: string; description: string; rows: UtmMetricRow[] }) {
...
export function getShareSource(platform: UtmPlatform | null | undefined) {
  if (!platform) return "unknown";
  return platform.source || platform.platform_key || "unknown";
}

export function RefPreview({ url }: { url: string }) {
...
export function BuilderTab({ config }: { config: UtmShareConfig }) {
...
export function MetricsTab({ data, isLoading }: { data: UtmPerformanceResponse | undefined; isLoading: boolean }) {
...
export function RecentSharesTab({ rows }: { rows: UtmRecentShareRow[] }) {
  if (!rows.length) {
    return (
      <Card>
