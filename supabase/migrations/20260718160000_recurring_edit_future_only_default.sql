-- Migration: Recurring amount edits are future-only by default.
-- Past generated instances and prepaid history stay unchanged unless
-- p_update_generated_instances is explicitly set to true.
...
CREATE OR REPLACE FUNCTION public.update_recurring_expense(
  ...
  p_update_generated_instances BOOLEAN DEFAULT FALSE
)
RETURNS JSONB
SECURITY DEFINER
...
BEGIN
  ...
  -- Update template only (drives future cycles)
  ...
  -- Optional: sync generated instances + prepaid (OFF by default — future-only)
...
COMMENT ON FUNCTION public.update_recurring_expense(UUID, NUMERIC, TEXT, TEXT, INTEGER, DATE, BOOLEAN, BOOLEAN) IS
  'Update recurring template (amount/description + splits) and schedule. Past generated instances stay unchanged by default (future-only). Pass p_update_generated_instances=true to also sync past instances and prepaid.';
