-- Add periods_covered to member prepaid info so UI can show
-- "1 payment covering 5 months" instead of only payment count.

CREATE OR REPLACE FUNCTION public.get_all_members_prepaid_info(
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      (SELECT SUM(rpp.periods_covered) FROM recurring_prepaid_payments rpp
       WHERE rpp.recurring_expense_id = p_recurring_expense_id
         AND rpp.user_id = es.user_id),
      0
    )::INTEGER AS periods_covered
...
COMMENT ON FUNCTION public.get_all_members_prepaid_info(UUID) IS
  'Get prepaid info for all members including total periods covered across payments.';

GRANT EXECUTE ON FUNCTION public.get_all_members_prepaid_info(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_all_members_prepaid_info(UUID) TO service_role;
