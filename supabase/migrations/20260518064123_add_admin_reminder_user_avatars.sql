-- Include debtor avatars in admin reminder summaries so the group-based tool remains scannable.
...
BEGIN
  ...
  user_debts AS (
    ...
    GROUP BY p.id, p.full_name, p.avatar_url, p.email, u.id, u.email, db.debt_breakdown, gb.group_breakdown
  )
...
COMMENT ON FUNCTION public.admin_get_email_devtool_summary(INT) IS
  'Admin-only email devtool summary. Debtor rows include dashboard-equivalent totals, counterparty debt context, grouped raw outstanding context, placeholder-account metadata, and recipient avatars for reminder emails.';
