-- Migration: Fix record_member_prepaid function
-- Issue: "record v_template is not assigned yet" error
-- Root cause: Cannot SELECT INTO a mix of scalar vars and record fields
...
BEGIN
  ...
  -- Get template expense details (use scalar variables, not record fields)
  SELECT
    e.currency,
    e.description,
    e.group_id,
  ...
  INSERT INTO expenses (
    ...
    v_description || ' (Prepaid ' || p_months || ' month' ||
