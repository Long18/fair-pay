-- Switch admin_merge_profiles from ban-orphan to claim/transfer identities
-- so any merged email/Google lands on the canonical auth user.
...
BEGIN
  ...
  IF FOUND THEN
    PERFORM public.transfer_auth_identities_to_user(p_source_user_id, p_target_user_id);

    PERFORM public.claim_auth_users_for_emails(
  ...
  RETURN jsonb_build_object(
    ...
    'source_auth_claimed', v_source_has_auth
  );
END;
$fn$;

COMMENT ON FUNCTION public.admin_merge_profiles(UUID, UUID) IS
  'Admin-only RPC to merge source into target. Dedupes unique-keyed rows, transfers source/orphan auth identities onto target (so secondary Google signs in as canonical), records merge transactions. Idempotent remarge returns noop.';

-- Repair already-merged accounts: move banned source identities onto targets
DO $repair$
DECLARE
  r RECORD;
  v_moved INT;
BEGIN
  FOR r IN
    SELECT pmt.source_user_id, pmt.target_user_id, pmt.source_emails
    FROM public.profile_merge_transactions pmt
  LOOP
    IF EXISTS (SELECT 1 FROM auth.users u WHERE u.id = r.source_user_id)
       AND EXISTS (SELECT 1 FROM auth.users u WHERE u.id = r.target_user_id)
       AND NOT EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = r.source_user_id)
    THEN
      v_moved := public.transfer_auth_identities_to_user(r.source_user_id, r.target_user_id);
      RAISE NOTICE 'repaired source % -> % moved %', r.source_user_id, r.target_user_id, v_moved;
    END IF;

    PERFORM public.claim_auth_users_for_emails(
      COALESCE(r.source_emails, '{}'::text[]),
      r.target_user_id,
      r.target_user_id
    );
  END LOOP;
