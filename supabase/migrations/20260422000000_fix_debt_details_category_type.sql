-- Fix get_user_debt_details: category column changed from TEXT to expense_category enum
-- Error: "Returned type expense_category does not match expected type text in column 3"
...
COMMENT ON FUNCTION get_user_debt_details(UUID, UUID) IS
  'Get detailed expense-level debt breakdown between two users. Returns each individual expense with description, date, amounts, settlement status, and context (group/friend).';
