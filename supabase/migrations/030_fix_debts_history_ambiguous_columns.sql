-- Migration: Fix Ambiguous Column References in get_user_debts_history
-- Description: Fixes the "column reference is ambiguous" error by properly qualifying
--              all column references in the debt_calculations CTE with table aliases.

BEGIN;

-- Drop the existing function first to avoid permission issues
...
COMMENT ON FUNCTION get_user_debts_history IS 'Returns all debt relationships for a user including settled debts, ordered by outstanding balance then recency. Fixed ambiguous column references.';
