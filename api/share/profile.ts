...
export default async function handler(req: Request): Promise<Response> {
  try {
    return await handleProfileShare(req)
  } catch (err) {
    console.error('[share/profile] unhandled error:', err)
    return simplePage('FairPay', 'Open FairPay to view this profile.')
  }
}

async function handleProfileShare(req: Request): Promise<Response> {
  if (req.method !== 'GET') {
  ...
  if (!id) {
    return simplePage('FairPay', 'Missing profile id')
  }

  const profile = await fetchProfileOgData(id)
  const redirectUrl = appendTrackingParams(
    `${base}/profile/${encodeURIComponent(id)}`,
    url,
  )
  const shareUrl = appendTrackingParams(
    `${base}/share/profiles/${encodeURIComponent(id)}`,
    url,
  )
  ...
  return shareLandingPage(
    {
      ...
      bodyText: 'Redirecting to FairPay profile page...',
